...
| 从旧版 plugin-starter 迁移           | [迁移旧版构建插件](#legacy-bundler-kit)        |
...
从 2.26.0 开始，`viteConfig` 和 `rsbuildConfig` 分别通过 `/vite` 和 `/rsbuild` 专用入口导出。2.27.0 移除了包根入口及 `HaloUIPluginBundlerKit`，已有项目请参考[迁移说明](#migration)。

从 2.27.0 开始，Vite、Rsbuild 及各自的 Vue 插件均为可选 peer 依赖。只需显式安装所选构建工具及其 Vue 插件，无需安装另一套工具链。以下安装示例使用 2.27.0。
...
## 迁移旧版构建插件 {#legacy-bundler-kit}

旧版本 [plugin-starter](https://github.com/halo-dev/plugin-starter) 使用的 `HaloUIPluginBundlerKit` 已在 2.27.0 中移除。升级前请按[迁移步骤](#migration)改用 `viteConfig` 或 `rsbuildConfig`。此变更不影响已有 IIFE 产物在 Halo 2.x 中的兼容性。
...
如果已经使用 `viteConfig` 或 `rsbuildConfig`，只需修改导入路径，并按上文安装所选构建工具及其 Vue 插件。现有配置参数和 Gradle 打包配置无需因此修改。
...
### 从 HaloUIPluginBundlerKit 迁移

如果当前插件使用旧版本 [plugin-starter](https://github.com/halo-dev/plugin-starter) 中的 `HaloUIPluginBundlerKit`，可以参考以下步骤迁移到 Vite 预配置：
