...
```bash
cp -r ~/.halo ~/.halo.1.4.13
```

```bash
cd ~/app && mv halo.jar halo.jar.1.4.13
```

3. 下载最新版本的运行包

```bash
cd ~/app && wget https://dl.halo.run/release/halo-1.4.14.jar -O halo.jar
```
...
```bash
docker pull halohub/halo:1.4.14
```
...
```bash
docker run -it -d --name halo -p 8090:8090 -v ~/.halo:/root/.halo --restart=unless-stopped halohub/halo:1.4.14
```
- **-it：** 开启输入功能并连接伪终端
- **-d：** 后台运行容器
- **--name：** 为容器指定一个名称
